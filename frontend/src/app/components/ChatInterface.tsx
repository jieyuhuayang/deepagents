...
export const ChatInterface = React.memo<ChatInterfaceProps>(({ assistant }) => {
  ...
  // When a single interrupt holds N action_requests for the same tool name
  // (e.g. 3 batched `task` sub-agent delegations), the UI only renders one
  // approval card. Broadcast the single decision to all N action_requests so
  // LangGraph's HumanInTheLoopMiddleware sees a length-matched resume payload.
  const broadcastResumeInterrupt = useCallback(
    (value: any) => {
      const ars =
        (interrupt?.value as any)?.action_requests as
          | Array<unknown>
          | undefined;
      const n = ars?.length ?? 0;
      if (n > 1 && Array.isArray(value?.decisions) && value.decisions.length === 1) {
        value = {
          ...value,
          decisions: Array.from({ length: n }, () => value.decisions[0]),
        };
      }
              ...
              {processedMessages.map((data, index) => {
                const messageUi = ui?.filter(
                  (u: any) =>
                    ...
                    data.toolCalls.some(
                      (tc) => tc.id === u.metadata?.tool_call_id
                    )
                );
                ...
                return (
                  <ChatMessage
                    ...
                    ui={messageUi}
                    stream={stream}
                    onResumeInterrupt={broadcastResumeInterrupt}
